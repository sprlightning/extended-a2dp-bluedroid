...
static const char *TAG = "lhdcv5BT_dec";
    ...
    uint8_t **pLout, uint32_t *pLlen, int upd_seq_no)
{
  ...
  if ((input == NULL) ||
      (pLout == NULL) ||
      (pLlen == NULL)) {
    ESP_LOGI(TAG, "%s: null ptr", __func__);
    return -1;
  }

  if (input_len < 2) {
    ESP_LOGI(TAG, "%s: input len too small", __func__);
    return -1;
  }
  ...
  if (status <= 0) {
    ESP_LOGI(TAG, "%s: no any frame in packet.", __func__);
    *frame_num = 0;
    return 0;
  }
  ...
  if (seqno != serial_no) {
    ESP_LOGI(TAG, "%s: packet lost! now(%d), expect(%d)", __func__, seqno, serial_no);
    //serial_no = seqno;
    //return -1;
  }
  ...
  *frame_num = (int) lhdc_total_frame_nb;
  ...
  ESP_LOGI(TAG, "%s: total frame number (%d)", __func__, *frame_num);
  return 0;
}
...
int32_t lhdcv5BT_dec_init_decoder(HANDLE_LHDCV5_BT *handle, tLHDCV5_DEC_CONFIG *config)
{
  ...
  ESP_LOGI(TAG, "%s: decoder lib version = %s", __func__, lhdcv5_util_dec_get_version());

  if (handle == NULL || config == NULL) {
    ESP_LOGI(TAG, "%s: null ptr handle %p config %p", __func__, handle, config);
    return LHDCV5BT_DEC_API_INVALID_INPUT;
  }

  ESP_LOGI(TAG, "%s: bits_depth:%u sample_rate=%u bit_rate=%u version=%d lossless_enable=%d", __func__,
  ...
  if ((config->bits_depth != LHDCV5BT_BIT_DEPTH_16) &&
    ...
    ESP_LOGI(TAG, "%s: bits_depth %d not supported", __func__, config->bits_depth);
    return LHDCV5BT_DEC_API_INVALID_INPUT;
  }
  ...
  if ((config->sample_rate != LHDCV5BT_SAMPLE_RATE_44K) &&
    ...
    ESP_LOGI(TAG, "%s: sample_rate %d not supported", __func__, config->sample_rate);
    return LHDCV5BT_DEC_API_INVALID_INPUT;
  }

  if ((0 > config->bit_rate) || (config->bit_rate > LHDCV5BT_BIT_RATE_1000K)) {
    ESP_LOGI(TAG, "%s: bit_rate %d not supported", __func__, config->bit_rate);
    return LHDCV5BT_DEC_API_INVALID_INPUT;
  }

  if ((config->version != VERSION_5)) {
    ESP_LOGI(TAG, "%s: version %d not supported", __func__, config->version);
    return LHDCV5BT_DEC_API_INVALID_INPUT;
  }

  if ((config->lossless_enable != 0) &&
      (config->lossless_enable != 1)) {
    ESP_LOGI(TAG, "%s: lossless enable invalid value %d ", __func__, config->lossless_enable);
    return LHDCV5BT_DEC_API_INVALID_INPUT;
  }
  ...
  if (func_ret != LHDCV5_UTIL_DEC_SUCCESS || mem_req_bytes <= 0) {
    ESP_LOGW(TAG, "%s: Fail to get required memory size (%d)!", __func__, func_ret);
    return LHDCV5BT_DEC_API_ALLOC_MEM_FAIL;
  }

  hLhdcBT = (HANDLE_LHDCV5_BT)malloc(mem_req_bytes);
  if (hLhdcBT == NULL) {
    ESP_LOGW(TAG, "%s: Fail to allocate memory!", __func__);
    return LHDCV5BT_DEC_API_ALLOC_MEM_FAIL;
  }

  ESP_LOGI(TAG, "%s: init lhdcv5 decoder...", __func__);
  ...
  if (func_ret != LHDCV5_UTIL_DEC_SUCCESS) {
    ESP_LOGW(TAG, "%s: failed to init decoder (%d)!", __func__, func_ret);
    free(hLhdcBT);
  ...
  *handle = hLhdcBT;
  if ((*handle) == NULL) {
    ESP_LOGW(TAG, "%s: handle return NULL!", __func__);
    return LHDCV5BT_DEC_API_INIT_DECODER_FAIL;
  }

  func_ret = lhdcv5_util_dec_channel_selsect(LHDC_OUTPUT_STEREO);
  if (func_ret != LHDCV5_UTIL_DEC_SUCCESS) {
    ESP_LOGW(TAG, "%s: failed to configure channel (%d)!", __func__, func_ret);
    return LHDCV5BT_DEC_API_CHANNEL_SETUP_FAIL;
  }

  serial_no = 0xff;

  ESP_LOGI(TAG, "%s: init lhdcv5 decoder success", __func__);
    ...
    uint32_t frameBytes, uint32_t *packetBytes)
{
  ...
  *packetBytes = 0;
  ...
  if (func_ret < 0 || in_buf == NULL) {
    ESP_LOGE(TAG, "%s: failed setup input buffer", __func__);
  ...
  ESP_LOGI(TAG, "%s: incoming frame size(%d), decoding size(%d), total frame num(%d)", __func__,
  ...
  while ((frame_num > 0) && (ptr_offset < in_len))
  {
    ...
    if (func_ret != LHDCV5_UTIL_DEC_SUCCESS) {
      ESP_LOGI(TAG, "%s: fetch frame info fail (%d)", __func__, func_ret);
      return LHDCV5BT_DEC_API_FRAME_INFO_FAIL;
    }

    ESP_LOGI(TAG, "%s: frame_num[%d]: ptr_offset (%d), frame_len (%d)", __func__,
        (int)frame_num, (int)ptr_offset, (int)lhdc_frame_Info.frame_len);

    if ((ptr_offset + lhdc_frame_Info.frame_len) > in_len) {
      ESP_LOGI(TAG, " %s: frame_num[%d]: Not Enough... ptr_offset(%d), frame_len(%d)",
    ...
    uint8_t *pcmData, uint32_t *pcmBytes, uint32_t bits_depth)
{
  ...
  ESP_LOGI(TAG, "%s: enter frameBytes %d", __func__, (int)frameBytes);
  ...
  *pcmBytes = 0;
  ...
  if(frameBytes >= 16) {
    for(int i=0; i<16; i++) {
      ESP_LOGI(TAG, " %s: dumpFrame[%d]= 0x%02X", __func__, i, (int)*(frameDataStart+i));
    }
  } else {
  ...
  if (func_ret != LHDCV5_UTIL_DEC_SUCCESS) {
    ESP_LOGI(TAG, "%s: fetch frame samples failed (%d)", __func__, func_ret);
    return LHDCV5BT_DEC_API_FRAME_INFO_FAIL;
  }
  ESP_LOGI(TAG, "%s: output frame samples %d", __func__, (int)frame_samples);
  ...
  while ((frame_num > 0) && (ptr_offset < in_len))
  {
    ...
    //ESP_LOGI(TAG, "%s: get ptr_offset=%d, dec_sum=%d", __func__, ptr_offset, dec_sum);
    func_ret = lhdcv5_util_dec_process(
        ...
        &lhdc_out_len);
    if (func_ret != LHDCV5_UTIL_DEC_SUCCESS) {
      ESP_LOGI(TAG, "%s: decode fail (%d)", __func__, func_ret);
      return LHDCV5BT_DEC_API_DECODE_FAIL;
    }

    ESP_LOGI(TAG, "%s: frame_num[%d]: input_frame_len %d output_len %d", __func__,
...
int32_t lhdcv5BT_dec_deinit_decoder(HANDLE_LHDCV5_BT handle)
{
  ...
  if(handle == NULL) {
    ESP_LOGI(TAG, "%s: empty handle", __func__);
    return LHDCV5BT_DEC_API_SUCCEED;
  }

  func_ret = lhdcv5_util_dec_destroy();
  if (func_ret != LHDCV5_UTIL_DEC_SUCCESS) {
    ESP_LOGI(TAG, "%s: deinit decoder error (%d)", __func__, func_ret);
    return LHDCV5BT_DEC_API_FAIL;
  }

  if(handle) {
    ESP_LOGI(TAG, "%s: free handle %p!", __func__, handle);
    free(handle);
  }
