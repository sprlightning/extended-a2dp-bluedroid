    ...
    q15_t* vec_buffer)
{
    ...
    while (rowCnt)
    {
        q31_t sum = NN_ROUND(out_shift);
        q31_t sum2 = NN_ROUND(out_shift);

        if (pBias) {
            sum = sum + ((q31_t)(*pBias++) << bias_shift);
            sum2 = sum2 + ((q31_t)(*pBias++) << bias_shift);
        }
    ...
    q15_t* vec_buffer)
{
    ...
    for (i = 0; i < num_of_rows; i++)
    {
        q63_t ip_out =  NN_ROUND(out_shift);

        if (bias) {
            ip_out = ip_out + ((q31_t)(bias[i]) << bias_shift);
        }
        for (j = 0; j < dim_vec; j++)
        {
            ip_out += pV[j] * pM[i * dim_vec + j];
