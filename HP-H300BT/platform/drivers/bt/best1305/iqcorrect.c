...
static int bt_IQ_DC_Mismatch_get_energy(IQMismatchPreprocessState *st, int fftsize, int* Energy)
{
...
calib_again:
    ...
    if (calib_time == 3) {
        BT_DRV_TRACE(0,"WARNING: %s get energy = 0", __func__);
    }
    ...
    if (Energy1 <= 5) {
        BTRF_REG_SET_FIELD(0x95, 0x3, 8, 0x3);
        calib_time++;
        BT_DRV_TRACE(0,"%s get energy = 0", __func__);
        goto calib_again;
    }
...
void DC_correction(IQMismatchPreprocessState *st,int *dc_i_r,int *dc_q_r,int fftsize,int i_min_val, int q_min_val, int dc_min_energy)
{
    ...
    for(k=0; k<dc_iters; k++)
    {
        ...
        bt_Txdc_cal_set(0,dc_i);
        bt_Txdc_cal_set(1,dc_q);
        Energy1 = bt_IQ_DC_Mismatch_get_energy(st, fftsize, &Energy1);
        P0 = Energy1;
#ifdef IQ_CALI_PRINT
...
void bt_IQ_DC_Mismatch_Correction_Release()
{
    ...
    // adc reset
    BTRF_REG_SET_FIELD(0xAE,0x1,11, 0x1);
    hal_sys_timer_delay(MS_TO_TICKS(100));
    BTRF_REG_SET_FIELD(0xAE,0x1,11, 0x0);
    hal_sys_timer_delay(MS_TO_TICKS(100));
