...
const struct attm_desc_128 datapathps_att_db[DATAPATHPS_IDX_NB] =
{
    ...
    [DATAPATHPS_IDX_TX_NTF_CFG] =   {ATT_DESC_CLIENT_CHAR_CFG_UUID, PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(WP, AUTH), 0,2},
#else
    [DATAPATHPS_IDX_TX_NTF_CFG] =   {ATT_DESC_CLIENT_CHAR_CFG_UUID, PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE), 0, 2},
#endif
...
#ifdef CTKD_ENABLE  // let app write ccc to trigger secure connection pairing
    // TX Characteristic - Client Characteristic Configuration Descriptor
    [DATAPATHPS_IDX_TX_NTF_CFG] =   {ATT_DESC_CLIENT_CHAR_CFG, PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(WP, AUTH), 0, 2},
#else
    [DATAPATHPS_IDX_TX_NTF_CFG] =   {ATT_DESC_CLIENT_CHAR_CFG, PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE), 0, 2},
#endif
