...
#ifdef SPOT_ENABLED
    [GFPSP_IDX_BEACON_ACTIONS_CHAR]             =   {ATT_DECL_CHARACTERISTIC_UUID, PERM(RD, ENABLE)|PERM(WRITE_REQ, ENABLE), 0, GFPSP_VAL_MAX_LEN},
    [GFPSP_IDX_BEACON_ACTIONS_VAL]              =   {ATT_CHAR_BEACON_ACTIONS,   PERM(NTF, ENABLE) |PERM(RD, ENABLE) |PERM(WRITE_REQ, ENABLE), PERM(RI, ENABLE) | PERM_VAL(UUID_LEN, PERM_UUID_128), GFPSP_VAL_MAX_LEN},
    [GFPSP_IDX_BEACON_ACTIONS_CFG]              =   {ATT_DESC_CLIENT_CHAR_CFG_UUID,  PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE), PERM(RI, ENABLE), GFPSP_VAL_MAX_LEN},
#endif
