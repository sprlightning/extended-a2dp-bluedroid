...
void gapm_key_init(void);

const uint8_t* gapm_get_private_key(void);

uint32_t gapm_gen_DHKey(const uint8_t *in_PubKey,const uint8_t *in_PrivateKey,uint8_t *out_DHKey);

uint32_t gapm_make_P256_key(uint8_t * out_public_key,uint8_t * out_private_key);

uint32_t gapm_set_p256_key(const uint8_t* in_public_key,const uint8_t* in_private_key);
