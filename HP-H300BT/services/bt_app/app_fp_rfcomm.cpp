...
#ifdef SPOT_ENABLED
void app_fp_msg_send_firmware_version(void)
{
    TRACE(1,"%s",__func__);
    FP_MESSAGE_STREAM_T req =
        {FP_MSG_GROUP_DEVICE_INFO,
    ...
    app_fp_rfcomm_send(( uint8_t * )&req, FP_MESSAGE_RESERVED_LEN + strlen(firware_revision));
}

void app_fp_msg_send_eddystone_identifier_state(void)
{
    TRACE(1,"%s",__func__);
    FP_MESSAGE_STREAM_T req =
        {FP_MSG_GROUP_DEVICE_INFO,
         ...
         0,
         24};
    uint32_t current_time =  btdrv_syn_get_curr_ticks()/2200;
    uint8_t time[4];
    ...
    app_fp_rfcomm_send(( uint8_t * )&req, FP_MESSAGE_RESERVED_LEN + 24);
}

static void app_fp_spot_event_handler(void)
{
    ...
    app_fp_rfcomm_send((uint8_t *)&req, FP_MESSAGE_RESERVED_LEN+7);
}
#endif
...
#endif
#ifdef SPOT_ENABLED
            app_fp_msg_send_firmware_version();
            app_fp_msg_send_eddystone_identifier_state();
#endif
        }
