...
#define COEFF_FOR_MIX_PROMPT_FOR_MUSIC    0.12
#define COEFF_FOR_MIX_PROMPT_FOR_CALL     0.12


#define COEFF_FOR_MIX_MUSIC_FOR_MUSIC    0.0
#define COEFF_FOR_MIX_CALL_FOR_CALL      0.0


static uint8_t resampler_buf[AUDIO_PROMPT_BUF_SIZE_FOR_RESAMPLER];
...
bool audio_prompt_start_playing(uint16_t promptPram, uint32_t targetSampleRate)
{
    ...
    bool isPlayingLocally = false;
        ...
        if(promptId >= MAX_RECORD_NUM)
        {
            TRACE(0, "................promptId error= %d",promptId);
            return false;
		}

		if (app_tws_ibrt_tws_link_connected())
		{
			if((promptId == AUD_ID_BT_CHARGE_PLEASE)||(promptId == AUD_ID_POWER_OFF)||(promptId == AUD_ID_BT_PAIRING_FAIL))
			{
            ...
            if (MIX_WITH_A2DP_STREAMING == audio_prompt_env.mixType)
            {
                for (uint32_t i = 0; i < src_len; i++) {
                    dst_buf[i] = (int16_t)((float)src_buf0[i]*COEFF_FOR_MIX_PROMPT_FOR_MUSIC) + src_buf1[i] -
                        (int16_t)((float)src_buf1[i]*COEFF_FOR_MIX_MUSIC_FOR_MUSIC);
                }    
            }
            else
            {
            ...
            if (MIX_WITH_A2DP_STREAMING == audio_prompt_env.mixType)
            {
                for (uint32_t i = 0; i < src_len; i++) {
                    dst_buf[i] = (int32_t)((float)((int32_t)src_buf0[i] << 8)*COEFF_FOR_MIX_PROMPT_FOR_MUSIC) + src_buf1[i] -
                        (int32_t)((float)src_buf1[i]*COEFF_FOR_MIX_MUSIC_FOR_MUSIC);
                }    
            }
            else
            {
