...
osThreadDef(communication_thread, osPriorityHigh, 1, 1024, "communication_server");
...
static void uart_rx_idle_timer_start(void)
{
    ...
    if (uart_rx_idle_timer_id != NULL) {
        ...
        osTimerStart(uart_rx_idle_timer_id, 5000);
    }
}
...
static void uart_rx_idle_handler(void const *param)
{
    ...
    if(uart_rx_idle_counter++ >= 3)//150 * 100 = 15s
    {
        TRACE(1,"[%s] enter...", __func__);
