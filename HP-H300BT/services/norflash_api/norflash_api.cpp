                ...
                )
{
	...
	if((mod_id == NORFLASH_API_MODULE_ID_FJH)||(mod_id == NORFLASH_API_MODULE_ID_BLEAPP))
	{
	   mod_info->mod_base_addr = TO_FLASH_NC_ADDR(mod_info->mod_base_addr);
	}
	#endif
	

    NORFLASH_API_TRACE(5,"%s: mod_id = %d,mod_info->mod_base_addr = 0x%x,mod_info->mod_len = 0x%x,async = %d.",
                __func__,mod_id,mod_info->mod_base_addr,mod_info->mod_len,async);
