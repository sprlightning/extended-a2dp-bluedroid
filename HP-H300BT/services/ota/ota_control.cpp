...
#endif
#define COMMON_OTA_BUFF_SIZE    4096
...
static uint8_t common_ota_buf[COMMON_OTA_BUFF_SIZE];
...
uint8_t* app_get_common_ota_databuf(void)
{
    return common_ota_buf;
}
...
void ota_control_reset_env(void)
{
    ...
    ota_control_env.dataBufferForBurning = app_get_common_ota_databuf();
}
        ...
        case OTA_COMMAND_GET_OTA_RESULT:
        {
            ...
            if (ota_control_env.alreadyReceivedDataSizeOfImage == ota_control_env.totalImageSize)
            {
                ...
                if (ret)
                {
                    ...
                    ota_tws_send_handle(OTA_RSP_RESULT, (uint8_t *)&ret, 1, IBRT_OTA_TWS_IMAGE_CRC_CMD);
#else
                    FLASH_OTA_BOOT_INFO_T otaBootInfo = {COPY_NEW_IMAGE, 
                                                        ota_control_env.totalImageSize, 
                                                        ota_control_env.crc32OfImage, 
                                                        new_image_copy_flash_offset};
                    app_update_ota_boot_info(&otaBootInfo);
