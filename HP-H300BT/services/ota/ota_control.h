...
#define LOG_DBG(str,...)   TRACE(0,LOG_TAG"" str, ##__VA_ARGS__)             // DEBUG OUTPUT
#define LOG_MSG(str,...)   TRACE(0,LOG_TAG"" str, ##__VA_ARGS__)             // MESSAGE OUTPUT
#define LOG_ERR(str,...)   TRACE(0,LOG_TAG"err:""" str, ##__VA_ARGS__)       // ERROR OUTPUT

#define LOG_FUNC_LINE()    TRACE(0,LOG_TAG"%s:%d\n", __FUNCTION__, __LINE__)
#define LOG_FUNC_IN()      TRACE(0,LOG_TAG"%s ++++\n", __FUNCTION__)
#define LOG_FUNC_OUT()     TRACE(0,LOG_TAG"%s ----\n", __FUNCTION__)

#define LOG_DUMP           DUMP8
#else
#define LOG_DBG(str,...)
#define LOG_MSG(str,...)   TRACE(0,LOG_TAG""str, ##__VA_ARGS__)
#define LOG_ERR(str,...)   TRACE(0,LOG_TAG"err:"""str, ##__VA_ARGS__)
...
#define NEW_IMAGE_FLASH_OFFSET    0xE7000
#endif
...
#ifndef OTA_DATA_BUFFER_SIZE_FOR_BURNING
...
typedef struct
{
    uint8_t     packetType;             // should be OTA_RSP_SET_USER
    uint8_t     isDone;    // 1 if the setting user has been done successfully, otherwise, 0
} __attribute__ ((__packed__)) OTA_RSP_USER_T;
...
typedef enum
{
    ...
    OTA_COMMAND_SET_USER      = 0x97,
    OTA_RSP_SET_USER      = 0x98,
...
void ota_handle_data(uint8_t *otaBuf, bool isViaBle, uint16_t len);
...
void ota_gattover_handle_received_data(uint8_t *otaBuf, bool isViaBle, uint16_t len);
