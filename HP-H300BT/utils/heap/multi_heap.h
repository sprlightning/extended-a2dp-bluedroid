...
/** @brief check malloc() a buffer in a given heap is available
 *
 * Semantics are the same as standard malloc(), only the returned buffer will be allocated in the specified heap.
 *
 * @param heap Handle to a registered heap.
 * @param size Size of desired buffer.
 *
 * @return true means available, or fail if allocation fails.
 */
...
/** @brief Dump heap information to callback
 *
 * For debugging purposes, this function dumps information about every block in the heap to callback.
 *
 * @param heap Handle to a registered heap.
 */
...
/** @brief using data watchpoint as the guard,there are only 4 watchpoint*/
enum memory_guard {
    MEMORY_GUARD_1,
    MEMORY_GUARD_2,
    MEMORY_GUARD_3,
...
/** @brief put a guard on the tail of one memory block
 *
 * @param heap Handle to a registered heap.
 * @param info Pointer to a structure to fill with heap metadata.
 * @param guard, the watchpoint number
 */
void multi_heap_enable_tail_guard(multi_heap_handle_t heap, void *p, enum memory_guard guard);

/** @brief disable the guard of one memory block
 *
 * @param heap Handle to a registered heap.
 * @param info Pointer to a structure to fill with heap metadata.
 * @param guard, the watchpoint number
