...
static uint32_t ota_check_whole_image_crc(uint32_t srcFlashOffset)
{
    ...
    while (verifiedDataSize < otaBootInfo.imageSize)
    {
        if (otaBootInfo.imageSize - verifiedDataSize > OTA_DATA_BUFFER_SIZE_FOR_BURNING)
        {
            verifiedBytes = OTA_DATA_BUFFER_SIZE_FOR_BURNING;
        }
        else
        {
            verifiedBytes = otaBootInfo.imageSize - verifiedDataSize;
        }
        ...
        crc32Value = crc32(crc32Value, (void*)srcbuf, verifiedBytes);
...
void ota_copy_compressed_image(uint32_t srcFlashOffset, uint32_t dstFlashOffset)
{
    ...
    readOffset += 4;
    crcValue = ota_check_whole_image_crc(srcFlashOffset);
    TRACE(2,"imageCrc:0x%x,0x%x",otaBootInfo.imageCrc,crcValue);
    if(otaBootInfo.imageCrc != crcValue)
    {
